...
function gpaFieldsChanged(current: FormState, initial: FormState): boolean {
  return current.gpa !== initial.gpa || current.sgpa !== initial.sgpa;
}

function datFieldsChanged(current: FormState, initial: FormState): boolean {
  if (current.dat_type !== initial.dat_type) return true;
  const keys = Object.keys(current.dat_scores) as DatFieldKey[];
  return keys.some((key) => current.dat_scores[key] !== initial.dat_scores[key]);
}
...
export interface ProfileDetailsEditorProps {
  student: Student;
  mode: ProfileEditMode;
  canEditName?: boolean;
  /** When true and saving academics, clear gpa_verified/dat_verified if those fields changed. */
  staffMode?: boolean;
  onSaved?: () => void;
  /** Render without Modal chrome; shows an inline Save button. */
  embedded?: boolean;
  /** Optional heading above the form when embedded. */
  title?: string;
  /** Modal mode: whether the dialog is open. */
  open?: boolean;
  /** Modal mode: close handler. */
  onClose?: () => void;
}
...
export function ProfileDetailsEditor({
  ...
  const [initialForm, setInitialForm] = useState<FormState>(() => buildForm(student));
  ...
  const isDirty = useMemo(() => {
    if (!formsEqual(form, initialForm)) return true;
    if (prevAppFile) return true;
    if (docIdsToDelete.length > 0) return true;
    return false;
  }, [form, initialForm, prevAppFile, docIdsToDelete]);

  const handleReset = () => {
    setForm(cloneForm(initialForm));
    setPrevAppFile(null);
    setDocIdsToDelete([]);
    // Restore existing previous-app doc preview if it was only marked for delete.
    if (initialForm.previous_application_doc_id && !existingPrevDoc) {
      void documentsApi
        .get(initialForm.previous_application_doc_id)
        .then((doc) => setExistingPrevDoc(doc))
        .catch(() => setExistingPrevDoc(null));
    }
  };

  useEffect(() => {
    if (!active || mode !== "academic") return;
  ...
  }, [active, mode, form.previous_application_doc_id]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (mode === "personal") {
        ...
        const staffVerifyPatch =
          staffMode
            ? {
                ...(gpaFieldsChanged(form, initialForm) ? { gpa_verified: false } : {}),
                ...(datFieldsChanged(form, initialForm) ? { dat_verified: false } : {}),
              }
            : {};
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {REAPPLICANT_OUTCOMES.map((outcome) => {
                    const activeOutcome = school.outcomes.includes(outcome);
                    return (
                      <button
                        key={outcome}
                        type="button"
