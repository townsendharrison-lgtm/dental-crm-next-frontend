...
export default function StudentInboxPage() {
  ...
  // Lock parent layout scrolling on mount
  useEffect(() => {
  ...
  // Load Directory Users: Assigned Mentor + Admins
  const loadDirectoryUsers = async () => {
    if (!currentUserId) return;
    try {
      // 1. Fetch student profile to get assigned mentor ID
      ...
      // 2. Fetch assigned mentor if present
      if (mentorId) {
      ...
      // 3. Fetch all mentors and filter for Admin/Managers
      ...
      setDirectoryUsers(merged.filter((u) => u.id !== currentUserId));
    } catch (err) {
      console.error("Failed to load student directory:", err);
      toast.error("Failed to load directory recipients.");
    }
  };
  ...
  const handleStartNewChat = async () => {
    ...
    try {
      ...
      const conv = await messagesApi.create({ participantIds });
  ...
  const handleRecipientToggle = (user: DirectoryUser) => {
    if (selectedRecipients.some((r) => r.id === user.id)) {
      ...
      // Students can only do 1-on-1 chats in this directory view
  ...
  const chatHeaderTitle = useMemo(() => {
    ...
    return displayParticipants[0]?.name || "Advisor Thread";
          ...
          <div className="p-6 border-b border-slate-800/60">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800/80 rounded-lg py-2.5 pl-11 pr-4 text-xs focus:outline-none focus:border-indigo-500/50 transition-all text-white placeholder:text-slate-600"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
            {loadingConv ? (
              <div className="flex flex-col items-center justify-center py-20 space-y-2">
                ...
                <p className="text-slate-600 text-[10px] uppercase font-bold tracking-widest">Loading</p>
              </div>
              ...
              filteredConversations.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 flex items-center gap-4 rounded-lg transition-all border border-transparent text-left group cursor-pointer ${
                      isSelected
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        <h4 className="font-bold text-sm text-slate-200 truncate group-hover:text-white transition-colors">
                          {c.is_group ? c.name || "Group Chat" : otherParticipant?.name || "Advisor Chat"}
                        </h4>
                      </div>
          ...
          {selectedConversationId && activeConversation ? (
              ...
              <div ref={messagesContainerRef} className="flex-1 px-6 py-6 overflow-y-auto custom-scrollbar flex flex-col gap-3 bg-slate-950/10">
                {loadingMsgs ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-600">
                    <p className="text-sm">No messages yet. Send a note to start!</p>
                  </div>
                ) : (
                  messages.map((msg, idx) => {
                    const isMe = msg.sender_id === currentUserId;
                    const sender = (activeConversation.participants || []).find((p) => p.id === msg.sender_id);
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
              ...
              <p className="text-slate-500 text-xs max-w-[240px] leading-relaxed">
                Choose a chat partner from the left menu to start messaging.
              </p>
            </div>
          )}
        </div>
            ...
            >
              <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white tracking-tight">Message Advisor</h3>
                ...
                <div className="max-h-[220px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {filteredDirectoryUsers.map((user) => (
                    <button
                      key={user.id}
                      onClick={() => handleRecipientToggle(user)}
                      className="w-full p-3 flex items-center justify-between hover:bg-indigo-600/10 rounded-lg transition-all border border-transparent hover:border-indigo-500/10 cursor-pointer"
                    >
                      <div className="flex items-center gap-3">
                  ...
                  {filteredDirectoryUsers.length === 0 && (
                    <p className="text-center py-6 text-slate-600 text-xs italic">No advisors found</p>
                  )}
                </div>
              </div>
            </motion.div>
